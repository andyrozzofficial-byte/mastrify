...
async function syncJobsFromFeedbackInner(): Promise<{ synced: number } | { error: string }> {
  ...
  if (!feedback?.length) return { synced: 0 }
...
async function fetchAdminFeedbackInner(): Promise<AdminFeedbackRow[] | { error: string }> {
  ...
  return (data ?? []).map((row) => mapAdminFeedbackRow(row as unknown as BetaFeedbackDbRow))
}

/** Lightweight feedback rows for panel rank/points (no full survey payload). */
export async function fetchAdminFeedbackMetaForEmail(
  email: string,
): Promise<Pick<AdminFeedbackRow, "id" | "session_id" | "created_at" | "admin_notes">[]> {
  const supabase = createSupabaseServerClient()
  if (!supabase) return []
  ...
  const { data, error } = await supabase
    .from(BETA_FEEDBACK_TABLE)
    .select("id, session_id, created_at, admin_notes")
...
/** Activity timestamps only — for active-day counts without full row payloads. */
export async function fetchAdminFeedbackDatesForEmail(email: string): Promise<string[]> {
  const supabase = createSupabaseServerClient()
  if (!supabase) return []
  const normalized = email.trim().toLowerCase()
  if (!normalized.includes("@")) return []
...
export async function fetchAdminFeedbackPaginated(
  ...
  const rows = (result.data ?? []).map((row) => mapAdminFeedbackRow(row as unknown as BetaFeedbackDbRow))
...
export async function fetchAdminFeedbackById(
  ...
  return mapAdminFeedbackRow(data as unknown as BetaFeedbackDbRow)
}
...
/** Support ticket timestamps only — for active-day counts. */
export async function fetchAdminSupportDatesForEmail(email: string): Promise<string[]> {
  const supabase = createSupabaseServerClient()
  if (!supabase) return []
  const normalized = email.trim().toLowerCase()
  if (!normalized.includes("@")) return []
