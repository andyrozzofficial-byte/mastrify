...
export default function HeroEngineOrb({
  ...
  return (
    <motion.div
      className={`relative isolate mx-auto w-full max-w-full min-w-0 overflow-hidden px-3 py-6 max-lg:mb-2 sm:px-4 sm:py-8 lg:overflow-visible lg:px-0 lg:py-0 ${className}`}
      initial={reduce ? false : { opacity: 0, scale: 0.97 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ duration: 0.9, delay: 0.06, ease: CINEMATIC_EASE }}
    >
      <motion.div className="relative mx-auto w-[min(18rem,78vw)] max-w-full overflow-hidden sm:w-[min(20rem,82vw)] md:w-[min(21rem,38vw)] lg:w-[min(24rem,42vw)] lg:max-w-[28rem] lg:overflow-visible">
