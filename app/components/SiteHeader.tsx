    ...
    >
      <div className="mx-auto w-full max-w-[1240px] min-w-0">
        <div className="flex h-[50px] min-w-0 items-center justify-between gap-2 px-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] min-[430px]:px-[max(0.875rem,env(safe-area-inset-left))] min-[430px]:pr-[max(0.875rem,env(safe-area-inset-right))] sm:h-12 sm:gap-3 sm:px-4 md:hidden">
          <Link
            href="/"
            className="min-w-0 shrink bg-gradient-to-r from-white via-purple-200 to-cyan-200/90 bg-clip-text text-[1.02rem] font-extrabold tracking-tight text-transparent sm:text-lg"
          >
            Mastrify
          </Link>

          <Link href="/master" className={`${navCtaClass} shrink-0`}>
            Start
          </Link>
        </div>
          ...
          {links.map(({ href, label, short }) => {
            ...
            return (
              <Link
                key={href}
                href={href}
                className={`relative flex min-h-[38px] min-w-0 flex-1 items-center justify-center rounded-lg px-1.5 py-2 text-[11px] font-medium tracking-wide transition active:scale-[0.98] min-[430px]:px-2 sm:min-h-[40px] sm:px-3.5 sm:text-[12px] ${
                  active
              ...
              {links.map(({ href, label }) => {
                ...
                return (
                  <Link
                    key={href}
                    href={href}
                    className={`relative inline-flex h-9 items-center text-[13px] font-medium leading-none tracking-wide transition hover:text-white/88 ${
                      active ? "text-white" : "text-white/62"
                    }`}
                  >
                    {label}
                    {active && (
                      <span className="pointer-events-none absolute bottom-0 left-1/2 h-[2px] w-[calc(100%+8px)] -translate-x-1/2 rounded-full bg-gradient-to-r from-transparent via-purple-400 to-transparent shadow-[0_0_10px_rgba(192,132,252,0.42)]" />
                    )}
                  </Link>
                )
              })}
