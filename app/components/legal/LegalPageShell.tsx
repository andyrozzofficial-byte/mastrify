...
export default function LegalPageShell({ label, title, lead, sections, children }: Props) {
  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 mx-auto w-full max-w-[640px] px-5 pb-20 pt-10 md:px-8 md:pb-24 md:pt-12">
        <motion.header
        ...
        >
          ...
          <p className="mt-5 text-[16px] leading-[1.7] text-muted md:text-[17px] md:leading-[1.75]">{lead}</p>
        </motion.header>
        ...
        <div className="space-y-0">
          {sections.map((section, i) => (
            <div key={section.title}>
              {i > 0 ? (
                <div
                  className="mb-11 h-px bg-gradient-to-r from-transparent via-white/[0.08] to-transparent md:mb-12"
                  aria-hidden
                />
              ) : null}
              <motion.section
                ...
                className="rounded-[1.15rem] border border-white/[0.08] bg-gradient-to-b from-white/[0.035] to-black/[0.32] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_16px_40px_rgba(0,0,0,0.28)] backdrop-blur-md md:p-7"
              >
                <h2 className="text-[17px] font-semibold leading-snug tracking-[-0.02em] text-white/92">
                  {section.title}
                </h2>
                <div
                  className={`mt-5 ${PROSE} ${section.narrow ? "mx-auto max-w-[32rem]" : "max-w-[36rem]"}`}
                >
                  {section.body}
                </div>
              </motion.section>
