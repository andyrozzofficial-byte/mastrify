...
import {
  btnMastrifySecondaryCore,
  btnResultPlay,
  btnResultPrimary,
  btnResultToggleActive,
  btnResultToggleInactive,
...
export default function MasterResultClient() {
    ...
    const restorePayment = async () => {
      ...
      if (checkout === "success" && sessionIdFromUrl) {
        ...
        if (result.paid && result.sessionId) {
          ...
          window.history.replaceState({}, "", "/master/result")
          return
        }
  ...
  }, [mounted, masterObjectKey, masteredUrl, setDeliveryEmail, setStripeSessionId])
  ...
  const waitForMobileReady = async (el: HTMLAudioElement) => {
  ...
  const seekMobileElementToTimeline = async (
    ...
    if (mobilePlaybackRequestRef.current !== requestId) return targetTime
    if (Math.abs(el.currentTime - targetTime) > 0.12) {
      try {
  ...
  const correctMobileTimelineAfterPlay = async (
    el: HTMLAudioElement,
    source: PreviewSource,
    absoluteSec: number,
    requestId: number
  ) => {
    if (mobilePlaybackRequestRef.current !== requestId) return
    const targetTime = timelineToElementTime(source, absoluteSec, el, true)
    ...
    const timers = [0, 60, 140, 280, 520]
    for (const delay of timers) window.setTimeout(correct, delay)
  ...
  const selectSource = async (next: "original" | "mastered") => {
    ...
    if (isMobileClient) {
      ...
      const requestId = mobilePlaybackRequestRef.current + 1
      mobilePlaybackRequestRef.current = requestId
      ...
      try {
        if (!audioElementHasSource(el, nextSrc)) {
          ...
          el.src = nextSrc
          el.load()
          await waitForMobileReady(el)
        } else {
          await waitForMobileReady(el)
        }
        if (mobilePlaybackRequestRef.current !== requestId) return
        await seekMobileElementToTimeline(el, next, abs, requestId)
        setPlayProgress(progressPercentFromAbsolute(abs))
        if (wasPlaying) {
          try {
            await el.play()
            await correctMobileTimelineAfterPlay(el, next, abs, requestId)
            setIsPlaying(true)
          } catch {
            setIsPlaying(false)
          }
  ...
  const togglePlayPause = async () => {
    ...
    if (isMobileClient) {
      ...
      try {
        ...
        if (mobilePlaybackRequestRef.current !== requestId) return
        await seekMobileElementToTimeline(el, selectedSource, timelineBeforePlay, requestId)
        el.volume = 1
        setPlayProgress(progressPercentFromAbsolute(timelineBeforePlay))
        await el.play()
        await correctMobileTimelineAfterPlay(el, selectedSource, timelineBeforePlay, requestId)
        setIsPlaying(true)
      } catch {
        setIsPlaying(false)
      } finally {
        if (mobilePlaybackRequestRef.current === requestId) {
          isSwappingMobileSourceRef.current = false
        }
      }
      return
    ...
    >
            ...
            <div className="fluid-surface card-pad-mobile flex flex-1 flex-col rounded-xl border border-white/[0.055] bg-black/[0.3] shadow-[inset_0_1px_0_rgba(255,255,255,0.035)] md:p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-1.5 sm:gap-2">
                <button
                  ...
                  className={`min-h-[44px] rounded-lg py-2.5 text-[11px] font-semibold transition-colors duration-200 sm:min-h-0 sm:py-2.5 sm:text-xs ${
            ...
            <div className="rounded-xl border border-white/[0.05] bg-black/[0.22] px-4 py-3 md:px-4 md:py-3.5">
              <div className="flex items-start justify-between gap-3">
                ...
                <Link
                  ...
                  className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-semibold ${btnMastrifySecondaryCore}`}
                >
                  Adjust
                </Link>
              </div>
      ...
      >
          ...
          <div className="w-full rounded-xl border border-white/[0.08] bg-white/[0.03] p-3">
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button
                ...
                className="shrink-0 rounded-lg border border-white/20 bg-transparent px-3 py-2 text-xs font-semibold text-white transition-colors duration-200 hover:border-violet-400/40 hover:bg-violet-600/10 disabled:opacity-50"
              >
                {promoLoading ? "…" : "Apply"}
              </button>
            </div>
        ...
        <Link
          ...
          className={`inline-flex min-h-[46px] w-full items-center justify-center rounded-xl px-6 text-[13px] font-semibold sm:min-h-[48px] sm:text-[14px] ${btnMastrifySecondaryCore}`}
        >
          New master
        </Link>
        <div className="mt-1.5 flex flex-col items-center border-t border-white/[0.06] pt-3">
          ...
          <button
            type="button"
            onClick={handleShare}
            className={`mt-2.5 flex min-h-[40px] min-w-[10.5rem] items-center justify-center gap-2 rounded-full px-4 text-[11px] font-semibold sm:text-xs ${btnMastrifySecondaryCore}`}
          >
          ...
          >
            ...
            <div className="mt-5 flex flex-col gap-2 sm:flex-row">
              ...
              <button
                ...
                disabled={deliverySending}
                className={`inline-flex min-h-[46px] w-full flex-1 items-center justify-center rounded-xl px-5 text-sm font-semibold text-white/70 ${btnMastrifySecondaryCore}`}
              >
                Not now
              </button>
            </div>
