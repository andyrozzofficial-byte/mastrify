...
function validate(body: unknown): { ok: true; data: BetaPostMasterQuickBody } | { ok: false; error: string } {
  ...
  const soundedGood = typeof b.soundedGood === "string" ? b.soundedGood.trim() : ""
  const couldImprove = typeof b.couldImprove === "string" ? b.couldImprove.trim() : ""
  if (!soundedGood) return { ok: false, error: "What sounded good is required" }
  if (!couldImprove) return { ok: false, error: "What could improve is required" }
