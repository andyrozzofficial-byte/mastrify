...
if (lufs < -13 && energy < 0.12) {
  energyLevel = "low"
} 
else if (lufs > -10 || energy > 0.28) {
  energyLevel = "high"
}
...
const lufsDiff = targetLufs - lufs
const lufsAdjustment = Math.max(0, Math.round(lufsDiff))

// 🔊 HEADROOM
const peakDb = peak > 0 ? 20 * Math.log10(peak) : -60

let headroomStatus = "OK"
if (peakDb > -1) headroomStatus = "Too high"
else if (peakDb < -6) headroomStatus = "Too low"
